...
const el = {
  ...
  assistantFocus: document.querySelector("#assistantFocus"),
  assistantCards: document.querySelector("#assistantCards"),
  assistantTips: document.querySelector("#assistantTips"),
...
function bindEvents() {
  ...
  document.querySelectorAll("[data-assistant-segment]").forEach((button) => {
    button.addEventListener("click", () => {
      activeSegment = button.dataset.assistantSegment;
      el.segmentFilterSelect.value = activeSegment;
      selectedInviteIds.clear();
      render();
      document.querySelector(".invite-panel")?.scrollIntoView({ behavior: "smooth", block: "start" });
    });
  });
...
function renderAssistant() {
  if (!el.assistantCards || !el.assistantTips) return;
  const insights = getAssistantInsights();
  el.assistantFocus.textContent = insights.focus;
  el.assistantCards.innerHTML = insights.cards.map((card) => `
    <button class="assistant-card ${card.level}" data-assistant-card-segment="${escapeAttribute(card.segment)}">
  ...
  el.assistantTips.innerHTML = insights.tips.length
    ? insights.tips.map((tip) => `
      <div class="assistant-tip ${tip.level}">
        <div>
          <strong>${escapeHtml(tip.title)}</strong>
          <p class="meta">${escapeHtml(tip.text)}</p>
        </div>
        ${tip.segment ? `<button class="secondary-button" data-assistant-card-segment="${escapeAttribute(tip.segment)}">${escapeHtml(tip.action || "Открыть")}</button>` : ""}
      </div>
    `).join("")
    : '<div class="assistant-tip"><strong>На сегодня критичных подсказок нет</strong><p class="meta">Можно планово добирать гостей на ближайшие игры.</p></div>';

  el.assistantCards.querySelectorAll("[data-assistant-card-segment]").forEach(bindAssistantSegmentButton);
  el.assistantTips.querySelectorAll("[data-assistant-card-segment]").forEach(bindAssistantSegmentButton);
}

function bindAssistantSegmentButton(button) {
  button.addEventListener("click", () => {
    const segment = button.dataset.assistantCardSegment;
    if (!segment) return;
    activeSegment = segment;
    el.segmentFilterSelect.value = activeSegment;
    selectedInviteIds.clear();
    render();
    document.querySelector(".split-layout")?.scrollIntoView({ behavior: "smooth", block: "start" });
  });
}

function getAssistantInsights() {
  const followupDue = state.clients.filter(isFollowUpDue);
  const instagramDue = state.clients.filter(isInstagramFollowUpDue);
  const repliedNoBooking = state.clients.filter((client) => (
    (getClientComm(client).replyStatus === "Ответил" || getInstagramComm(client).replyStatus === "Ответил") &&
    !hasActiveBooking(client.id)
  ));
  const warmInvite = state.clients.filter((client) => isRecentVisitor(client) && !hasActiveBooking(client.id));
  const notMessagedInstagram = state.clients.filter((client) => (
    client.instagram && getInstagramComm(client).status === "Не писали" && !hasActiveBooking(client.id)
  ));
  const lowTables = getLowTables();
  const selectedClient = state.clients.find((client) => client.id === selectedClientId);
  const selectedTip = selectedClient ? getClientAssistantTip(selectedClient) : null;
  const tips = [];

  if (followupDue.length) {
    tips.push({
      level: "urgent",
      title: "Есть просроченные касания",
      text: `${followupDue.length} клиентам нужно написать сегодня или раньше. Начать лучше с них.`,
      action: "Открыть",
      segment: "followupDue",
    });
  }
  ...
  if (lowTables.length) {
    const next = lowTables[0];
    tips.push({
      level: "warn",
      title: "Есть недобор за столами",
      text: `${lowTables.length} столов ниже минимума 5 участников. Ближайший: ${formatGame(next.game)}, ${next.table.name}, сейчас ${next.count}/${next.table.min}.`,
      action: "Добрать",
      segment: "recentVisitors",
    });
  }

  if (repliedNoBooking.length) {
    tips.push({
      level: "warn",
      title: "Ответили, но не записаны",
      text: `${repliedNoBooking.length} клиентов уже ответили, но пока не стоят на игре. Здесь легко потерять теплый интерес.`,
      action: "Разобрать",
      segment: "replied",
    });
  }

  if (instagramDue.length) {
    tips.push({
      level: "warn",
      title: "Instagram требует внимания",
      text: `${instagramDue.length} Instagram-контактов с наступившей датой следующего касания.`,
      action: "Открыть IG",
      segment: "instagramDue",
    });
  }

  if (selectedTip) tips.push(selectedTip);

  if (!tips.length && warmInvite.length) {
    tips.push({
      level: "ok",
      title: "Можно усилить ближайшую игру",
      text: `${warmInvite.length} теплых гостей недавно были на игре и сейчас не записаны.`,
      action: "Кого позвать",
      segment: "recentVisitors",
    });
  }
  ...
  const focusParts = [];
  if (followupDue.length) focusParts.push(`${followupDue.length} касаний`);
  if (lowTables.length) focusParts.push(`${lowTables.length} столов с недобором`);
  if (repliedNoBooking.length) focusParts.push(`${repliedNoBooking.length} ответили без записи`);
  if (!focusParts.length) focusParts.push("плановый добор гостей");

  return {
    focus: `Фокус дня: ${focusParts.join(", ")}.`,
    cards: [
      {
        label: "Написать сегодня",
        value: followupDue.length,
        note: "общие follow-up",
        level: followupDue.length ? "urgent" : "ok",
        segment: "followupDue",
      },
      {
        label: "Недобор столов",
        value: lowTables.length,
        note: "меньше 5 участников",
        level: lowTables.length ? "warn" : "ok",
        segment: "recentVisitors",
      },
      {
        label: "Ответили без записи",
        value: repliedNoBooking.length,
        note: "нужно довести до даты",
        level: repliedNoBooking.length ? "warn" : "ok",
        segment: "replied",
      },
      {
        label: "Instagram сегодня",
        value: instagramDue.length,
        note: `${notMessagedInstagram.length} еще не писали`,
        level: instagramDue.length ? "warn" : "ok",
        segment: "instagramDue",
      },
    ],
    tips: tips.slice(0, 5),
...
function getLowTables() {
  return state.games
    ...
    .filter((item) => item.count > 0 && item.count < Number(item.table.min || 5))
    .sort((a, b) => `${a.game.date} ${a.game.time}`.localeCompare(`${b.game.date} ${b.game.time}`));
}

function getClientAssistantTip(client) {
  if (hasActiveBooking(client.id)) {
    return {
      level: "ok",
      title: `По клиенту: ${client.name}`,
      text: "Клиент уже записан. Проверьте, есть ли напоминание и подтверждение прихода.",
      action: "Задачи",
      segment: "all",
    };
  }

  if (getClientComm(client).replyStatus === "Ответил" || getInstagramComm(client).replyStatus === "Ответил") {
    return {
      level: "warn",
      title: `По клиенту: ${client.name}`,
      text: "Клиент ответил, но не записан на игру. Следующий шаг: предложить конкретную дату и стол.",
      action: "Ответили",
      segment: "replied",
    };
  }

  if (client.instagram && getInstagramComm(client).status === "Не писали") {
    return {
      level: "warn",
      title: `По клиенту: ${client.name}`,
      text: "Есть Instagram, но еще не отмечена отправка. Можно написать вручную и зафиксировать касание.",
      action: "Instagram",
      segment: "instagramNotMessaged",
    };
  }

  if (isRecentVisitor(client) && !hasActiveBooking(client.id)) {
    return {
      level: "ok",
      title: `По клиенту: ${client.name}`,
      text: "Недавно был на игре и сейчас не записан. Подходит для мягкого повторного приглашения.",
      action: "Кого позвать",
      segment: "recentVisitors",
    };
  }

  if (isFollowUpDue(client) || isInstagramFollowUpDue(client)) {
    return {
      level: "urgent",
      title: `По клиенту: ${client.name}`,
      text: "Наступила дата следующего контакта. Лучше обработать сегодня.",
      action: "Сегодня",
      segment: isInstagramFollowUpDue(client) ? "instagramDue" : "followupDue",
    };
  }
