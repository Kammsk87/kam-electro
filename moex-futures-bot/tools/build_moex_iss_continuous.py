#!/usr/bin/env python3
"""Build continuous futures chains from MOEX ISS asset history."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--method", choices=("volume_leader", "sticky_volume_leader"), default="sticky_volume_leader")
...
def main() -> int:
    ...
    rows = con.execute(
        f"""
        SELECT tradedate, secid, shortname, assetcode, open, high, low, close, settleprice, volume
    ...
    con.close()
    if not rows:
        print(f"No rows for assetcode={args.assetcode}", file=sys.stderr)
        return 1

    chain_rows = _build_chain(args.assetcode, rows, args)
    out_path = paths.moex_iss_continuous_root / f"method={args.method}" / f"assetcode={safe_symbol(args.assetcode)}" / f"price={args.price_field}" / "bars.parquet"
...
def _build_chain(assetcode: str, rows, args: argparse.Namespace) -> list[dict[str, object]]:
    ...
    for row in rows:
        tradedate, secid, shortname, _, open_, high, low, close, settleprice, volume = row
        price = float(settleprice if args.price_field == "settleprice" else close)
    ...
    for tradedate in sorted(by_date):
        ...
        chain.append(
            {
                "assetcode": assetcode,
                "symbol": f"{assetcode}_continuous@MOEX_ISS",
...
def _write_report(root: Path, summary: dict[str, object]) -> Path:
    ...
    path = root / f"moex_iss_continuous_{summary['assetcode']}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    source_days = ", ".join(f"{symbol}: {days}" for symbol, days in summary["source_symbol_days"].items())
    lines = [
        "# MOEX ISS Continuous Chain",
        "",
        ...
        "",
        "Notes:",
        "- This chain uses MOEX ISS public history.",
        "- It is not back-adjusted.",
        "- Roll gaps must be audited before strong strategy conclusions.",
    ]
