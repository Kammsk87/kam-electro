#!/usr/bin/env python3
"""Build return-stitched MOEX ISS continuous futures chains."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--source-method", default="sticky_volume_leader")
    parser.add_argument("--output-method", default="sticky_volume_leader_return_stitched")
...
def main() -> int:
    ...
    try:
        ...
        if not chain:
            print("No source chain rows found.", file=sys.stderr)
            return 1
        history = _history(con, paths, args)
    finally:
        con.close()

    stitched, diagnostics = _stitch(chain, history, args)
    out_path = (
    ...
    try:
        record_data_inventory(
            sqlite_conn,
            source="moex_iss",
            dataset="return_stitched_continuous_bars",
...
def _history(con, paths, args: argparse.Namespace) -> dict[tuple[str, str], dict[str, float]]:
    glob = str(paths.moex_iss_history_root / "security=*" / "history.parquet")
    rows = con.execute(
        f"""
        SELECT tradedate, secid, open, high, low, {args.price_field} AS close, volume
...
def _stitch(
    ...
    for index, row in enumerate(chain):
        ...
        raw = history.get((symbol, date_key))
        if raw is None:
        ...
        if index == 0:
            ...
            prev_same_symbol = history.get((symbol, prev_date_key))
        ...
        stitched.append(
            {
                ...
                "symbol": f"{args.assetcode}_return_stitched@MOEX_ISS",
...
def _write_report(root: Path, summary: dict[str, object]) -> Path:
    ...
    path = root / f"moex_iss_return_stitched_{summary['assetcode']}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
