...
test('the seed laws are correctly typed and none overclaims its status', () => {
  ...
  assert(laws.length === 7, `expected seven seed laws, got ${laws.length}`);
  ...
  assert(empirical.length === 6, `expected six empirical laws, got ${empirical.length}`);
...
test('the seed registry closes methods as well as families', () => {
  ...
  assert(reg.length === 13, `expected thirteen seed decisions, got ${reg.length}`);
...
test('query "closures" orders measured before underpowered and reports both criteria', () => {
  ...
  assert(rows.length === 13, 'every decision is returned');
  ...
  assert(methodsOnly.length === 4 && methodsOnly.every((d) => d.subject_kind === 'METHOD'), 'the kind filter works');
});
