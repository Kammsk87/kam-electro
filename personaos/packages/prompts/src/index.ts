export type WritingPlatform = "TELEGRAM" | "INSTAGRAM" | "THREADS" | "VK";
...
export const writingPlatformGuidance: Record<WritingPlatform, string> = {
  TELEGRAM:
...
export function buildWritingPrompt(source: StoryPromptSource, platform: WritingPlatform) {
  return {
    system: [
      ...
      writingPlatformGuidance[platform]
    ].join("\n"),
    user: [
...
export function buildRewritePrompt(input: {
  ...
  return {
    system: [
      ...
      writingPlatformGuidance[input.platform]
    ].join("\n"),
    user: [
...
export const promptsStatus = "writing-engine-ready" as const;
